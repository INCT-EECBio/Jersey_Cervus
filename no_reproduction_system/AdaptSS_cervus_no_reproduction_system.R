...
run_generation <- function(Dmax, 
  ...
  for(t in 1:time){
    ...
    #Shifting peak
    Isl.Peak <-as.numeric(c(1,(Np[t]/mean(K.isl)))%*%coef.peak)
    ######################################################################
    Isl.Peak <-ifelse(Isl.Peak < 39,newP,Isl.Peak) # tem que tornar geral
    ######################################################################
    Isl.Peak <-ifelse(Isl.Peak > oldP,oldP,Isl.Peak)
    
    bp <-as.numeric(c(1,(Np[t]/mean(K.isl)))%*%coef.bp)
    
    #Fecundity by generation, demographic stochasticity
    fecund <-rpois(1, 7)
    
    res <- try(AdaptSS(G,h2m,vpi,w2,oldP,Isl.Peak,fecund,round(K.isl[t]),coef_ID,input_wrigth,Nrecol,Precol,vm, bp))
